...
export function BakeryOrderManagement() {
  ...
  const [newItem, setNewItem] = useState<Omit<Item, 'id' | 'visible' | 'currentQuantity'>>({
    name: '',
    category: '',
    parLevel: 0,
    image: ''
  ...
  const addItem = useCallback(() => {
    setItems(prevItems => [...prevItems, { ...newItem, id: Date.now(), visible: true, currentQuantity: 0 }])
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <Select value={darkMode} onValueChange={(value: 'light' | 'dark' | 'auto') => setDarkMode(value)}>
          <SelectTrigger className="w-[40px] px-2">
            <SelectValue placeholder={darkMode === 'dark' ? <Moon className="h-4 w-4" /> : <Sun className="h-4 w-4" />} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="light"><Sun className="mr-2 h-4 w-4" /></SelectItem>
            <SelectItem value="dark"><Moon className="mr-2 h-4 w-4" /></SelectItem>
      ...
      <Tabs defaultValue="todo" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          ...
          <TabsTrigger value="inventory">Inventory and Order Creation</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="inventory">
          <div className="space-y-4">
            {Object.entries(groupItemsByCategory()).map(([category, categoryItems]) => (
              <div key={category}>
                <h2 className="text-xl font-semibold mt-4 mb-2">{category}</h2>
                {categoryItems.filter(item => item.visible).map(item => (
                  <Card key={item.id} className="shadow-md hover:shadow-lg transition-shadow duration-300 mb-2">
                    <CardContent className="flex items-center justify-between p-4">
                      <div className="flex items-center space-x-4">
                        {item.image ? (
                          <img src={item.image} alt={item.name} className="w-12 h-12 object-cover rounded" />
                      ...
                      <Input
                        ...
                        onChange={(e) => updateCurrentQuantity(item.id, Number(e.target.value))}
                        className="w-24"
                      />
                    </CardContent>
                  </Card>
            ...
            <Button onClick={() => setIsCreateOrderOpen(true)}>Create Order</Button>
          </div>
        </TabsContent>
      </Tabs>
      ...
      <Dialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
        <DialogContent className="bg-background text-foreground">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => itemToDelete && deleteItem(itemToDelete)}>Delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          <div className="space-y-4">
            {Object.entries(groupItemsByCategory()).map(([category, categoryItems]) => (
              <div key={category}>
                <h2 className="text-xl font-semibold mt-4 mb-2">{category}</h2>
                {categoryItems.filter(item => item.visible).map(item => {
                  const orderQuantity = Math.max(0, item.parLevel - item.currentQuantity)
                  return (
                    <Card key={item.id} className="shadow-sm mb-2">
                      <CardContent className="flex items-center justify-between p-4">
                        ...
                        <div className="text-right">
                          ...
                          <p className="font-bold">Order Quantity: {orderQuantity}</p>
                        </div>
                      </CardContent>
                    </Card>
                  )
